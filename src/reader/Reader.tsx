...
import type { Book, Chapter, KnownWord, LearningStatus, Paragraph, Sentence, Token } from '../database/types';
...
import { graphemesOf } from '../tokenizer/devanagari';
...
export function Reader() {
  const { bookId: bookIdS, chapterId: chapterIdS } = useParams();
  const bookId = Number(bookIdS);
  const chapterId = Number(chapterIdS);
  ...
  const liveChapter = useRef(chapterId);
  ...
  const load = useCallback(async () => {
    liveChapter.current = chapterId;
    const book = await db.books.get(bookId);
    const chapter = await db.chapters.get(chapterId);
    if (!book || !chapter || chapter.book_id !== bookId) throw new Error('This chapter does not exist.');
    const chapters = (await db.chapters.where('book_id').equals(bookId).toArray()).sort((a, b) => a.index - b.index);
    const paragraphs = (await db.chapters.get(chapterId)) ? (await db.paragraphs.where('chapter_id').equals(chapterId).toArray()).sort((a, b) => a.index - b.index) : [];
    ...
    if (liveChapter.current !== chapterId) return;
    paraById.current = new Map(paragraphs.map((p) => [p.id!, p]));
    setSentenceCorrections(sc);
    setLoaded({ book, chapter, chapters, paragraphs, sentencesByPara, sentenceById, analysis });
    ...
    // jump to a sentence (from a lexeme page or a script lesson), else restore position
      ...
      (entries) => {
        for (const e of entries) {
          ...
          if (e.isIntersecting) seen.add(id);
            ...
            void recordContext(items.flatMap((it) => graphemesOf(it.form).map((g) => g.symbol))).catch(() => {});
  ...
  }, [loaded, bookId, chapterId, settings.autoKnownAfter]);
    ...
    <div className={`reader${panel ? ' reader--panel' : ''}`}>
      <div className="reader__main">
        <header className="reader__header">
          <div className="reader__header-inner">
            ...
            <button className="reader__headbtn" onClick={() => setToc(true)} aria-label="Chapters">
              ...
              <span className="reader__chapter">{chapter.title}</span>
            </button>
            <IconBtn label="Chapter preparation" to={`/prep/${bookId}/${chapterId}`}>{I.sparkle}</IconBtn>
            <IconBtn label={settings.translit === 'never' ? 'Transliteration: off' : `Transliteration: ${settings.translit}`} active={settings.translit !== 'never'} onClick={() => setSettings({ translit: settings.translit === 'never' ? 'tap' : settings.translit === 'tap' ? 'unknown' : settings.translit === 'unknown' ? 'always' : 'never' })}>{I.translit}</IconBtn>
        ...
        <div className={settings.showMarks ? 'marks' : ''}>
          <div style={{ maxWidth: 'var(--measure)', margin: '0 auto', padding: '2rem 1.35rem 0' }}>
            ...
            <div className="reader__chtitle dv">{chapter.title}</div>
          </div>
          ...
          <div className="reader__end">
            {next ? <Link to={`/read/${bookId}/${next.id}`}>Next: <span className="dv">{next.title}</span></Link> : 'End of the book.'}
        ...
        <nav className="reader__nav" aria-label="Chapter navigation">
          <div className="reader__nav-inner">
            <button className="reader__nav-btn" disabled={!prev} onClick={() => prev && nav(`/read/${bookId}/${prev.id}`)}><span className="reader__nav-dir">Previous</span><span className="reader__nav-cite">{prev?.title ?? '—'}</span></button>
            <button className="reader__nav-btn reader__nav-btn--next" disabled={!next} onClick={() => next && nav(`/read/${bookId}/${next.id}`)}><span className="reader__nav-dir">Next</span><span className="reader__nav-cite">{next?.title ?? '—'}</span></button>
          </div>
        </nav>
      </div>

      {panel === 'word' && selToken && selAt && selAnalysis && selSentence && (
        <WordPanel key={selToken.id} token={selToken} sentence={selSentence} analysis={selAnalysis} at={selAt} bookId={bookId} chapterId={chapterId} status={selKey ? statuses.get(selKey) : undefined} settings={settings} known={known} onClose={closePanel} onOpenSentence={() => setPanel('sentence')} onHighlightConstruction={highlightConstruction} onStatus={onStatus} onCorrected={() => void reanalyse()} />
      )}
      {panel === 'sentence' && selSentence && selAnalysis && (
        <SentencePanel key={selSentence.id} sentence={selSentence} tokens={selTokens} analysis={selAnalysis} bookId={bookId} chapterId={chapterId} statuses={statuses} settings={settings} selectedToken={sel.tokenId} correction={sentenceCorrections.get(selSentence.id!)} onClose={closePanel} onTapWord={(t) => onTapWord(t, selSentence)} onHighlightConstruction={highlightConstruction} onCorrected={() => void reanalyse()} />
      )}
      ...
      {toc && (
        <Sheet title="Chapters" onClose={() => setToc(false)}>
          <ul className="toc">
            {chapters.map((c) => (
              <li key={c.id}><button className={c.id === chapter.id ? 'on' : ''} onClick={() => { setToc(false); nav(`/read/${bookId}/${c.id}`); }}><span className="n">{c.index + 1}</span>{c.title}</button></li>
            ))}
          </ul>
        </Sheet>
      )}
      {typeSheet && (
        <Sheet title="Type & assistance" onClose={() => setTypeSheet(false)}>
          ...
          <div className="sheet__row"><label>Face</label><div className="segmented">{(['tiro', 'noto', 'system'] as const).map((f) => <button key={f} aria-pressed={settings.hindiFont === f} onClick={() => setSettings({ hindiFont: f })}>{f === 'tiro' ? 'Tiro' : f === 'noto' ? 'Noto Sans' : 'System'}</button>)}</div></div>
          ...
          <div className="sheet__row"><label>Transliteration</label><div className="segmented">{(['always', 'tap', 'unknown', 'never'] as const).map((t) => <button key={t} aria-pressed={settings.translit === t} onClick={() => setSettings({ translit: t })}>{t === 'unknown' ? 'unknown script only' : t === 'tap' ? 'on tap' : t}</button>)}</div></div>
